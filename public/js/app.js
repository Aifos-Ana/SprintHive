function addEventListeners() {
  ...
  document.querySelector('.search-task').addEventListener('input', function (e) {
    ...
    const projectId = document.querySelector('meta[name="csrf-token"]').content;
    ...
    fetch(`/projects/${projectId}?search=${encodeURIComponent(searchQuery)}`, {
        headers: {
        ...
        .then(data => {
            ...
            data.tasks.forEach(task => {
                ...
                taskElement.innerHTML = `
                    <h3><a href="/tasks/${task.taskid}" class="nav-link">${task.title}</a></h3>
